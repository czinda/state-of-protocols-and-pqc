...
| **PQ Signatures in TLS** | IETF LAST CALL | ML-DSA TLS v-03 in IETF Last Call (ends Jun 1); sig size crisis unsolved |
| **PKI / Certificates** | ~95% READY | 3 new RFCs (9935, 9936, 9964); **composite sigs cleared IESG → RFC Ed Queue** |
| **IPsec / IKEv2** | AD EVALUATION | ML-KEM moved to IESG; PQC auth write-up in progress |
| **SSH** | RFC ED QUEUE | ML-KEM hybrid in active RFC Editor processing; NTRU Prime published (RFC 9941) |
| **OpenPGP** | AUTH48 → RFC 9980 | Assigned RFC 9980. In final author review since May 8. Publication imminent. |
...
**Bottom line:** The composite vs. non-composite debate is being resolved through the standards process — composite signatures cleared IESG and are heading for RFC publication. Three more algorithm-binding RFCs published since March. KEM-based key exchange remains deployed at scale. The PQ signature size crisis is the critical unsolved challenge, but ML-DSA in TLS reaching IETF Last Call signals the WG is committing to a path despite the overhead.

---

## Changes Since March 2026

| Item | March 2026 Status | May 2026 Status | Significance |
|------|-------------------|-----------------|--------------|
| **Composite ML-DSA Sigs** | IESG Evaluation (v-15) | **RFC Ed Queue (v-19)** | Cleared IESG; 4 revisions to address review comments |
| **CMS Composite Sigs** | IETF Last Call (v-04) | **RFC Ed Queue (v-05)** | Cleared IETF LC + IESG; blocked on normative refs |
| **CMS Composite KEM** | WG Document (v-00) | **WG Last Call (v-01)** | Entered WG Last Call |
| **ML-DSA COSE/JOSE** | RFC Ed Queue (v-11) | **RFC 9964 Published** | New published RFC |
| **ML-KEM X.509** | IESG Approved | **RFC 9935 Published** | New published RFC |
| **ML-KEM CMS** | IESG Ballot | **RFC 9936 Published** | New published RFC |
| **ML-DSA TLS** | WG Document (v-02) | **IETF Last Call (v-03)** | Major advancement; LC ends Jun 1 |
| **SSH ML-KEM Hybrid** | IETF Last Call (v-10) | **RFC Ed Queue (v-10)** | Publication imminent |
| **IKEv2 ML-KEM** | Publication Requested (v-05) | **AD Evaluation (v-05)** | Moved to IESG evaluation |
| **MLS Combiner** | WG Last Call (v-02) | **Expired** | Expired Apr 25; needs new revision |
| **CFRG Hybrid KEMs** | Active RG Doc (v-10) | Active RG Doc (v-11) | One new revision |
| **Merkle Tree Certs** | WG Doc (v-02) | WG Doc (v-04) | Fresh revision May 24 |
| **OpenPGP PQC** | RFC Ed Queue (v-17) | **AUTH48 → RFC 9980** | Assigned RFC number; awaiting author approvals |
| **Composite ML-KEM** | Publication Requested (v-14) | Publication Requested (v-14) | **Stalled** — awaiting AD (55+ days) |
...
### Composite Standards Pipeline — May 2026
...
| **Composite ML-KEM** (lamps-pq-composite-kem) | X.509/PKI | Publication Requested (v-14) | Standards | AD evaluation (stalled 55+ days) |
| **CMS Composite Sigs** (lamps-cms-composite-sigs) | CMS | **RFC Ed Queue** (v-05) | Standards | Unblock normative refs → publish |
| **CMS Composite KEM** (lamps-cms-composite-kem) | CMS | **WG Last Call** (v-01) | Standards | Complete WG LC → IESG |
...
|----------|-------|--------|------------|
| **TLS** | ECDHE-MLKEM | RFC Ed Queue (v-04) | ~60% of web traffic |
| **TLS** | Hybrid KEX Framework | RFC Ed Queue (v-16) | Foundational (Informational) |
| **SSH** | ML-KEM Hybrid KEX | RFC Ed Queue (v-10) | Publication imminent |
| **IPsec** | ML-KEM in IKEv2 | AD Evaluation (v-05) | 4 implementations |
...
| | **Stalled 55+ days awaiting AD processing** |
...
**Status:** RFC Ed Queue, v-04 (Feb 2026), **Standards Track**
...
**Status:** **IETF Last Call** (v-03, May 2026), Last Call ends June 1, 2026. **Major advancement since March.**

| Pros | Cons |
|------|------|
| NIST's primary PQ signature standard | ML-DSA-65 signatures: 3,309 bytes |
| **In IETF Last Call — submitted to IESG** | Certificate chains add ~17 KB to handshakes |
| GENART review: "Ready" | Exceeds TCP initcwnd (~14 KB), adds round-trips |
...
**Status:** RFC Ed Queue, v-16 (Sep 2025), Informational
...
| Standards Track, Google+Cloudflare backing | Active development (v-04 May 24) |
...
| Draft | Status | Notes |
|-------|--------|-------|
| **draft-ietf-lamps-pq-composite-kem** v-14 | Publication Requested | Composite ML-KEM. **Stalled 55+ days awaiting AD.** |
| **draft-ietf-lamps-cms-composite-kem** v-01 | **In WG Last Call** | Composite ML-KEM in CMS. Advanced from v-00. |
...
**Status:** **AD Evaluation** (v-05, Mar 2026), Standards Track. Advanced from publication requested.
...
**Status:** **AUTH48 → RFC 9980**, Proposed Standard, v-17 (Jan 2026). **Publication imminent.**

Assigned RFC 9980. In AUTH48 final author review since May 8, 2026. Awaiting approval from all four authors (Kousidis, Roth, Strenzke, Wussler) and AD (Paul Wouters) response to an editorial question. No normative reference blockers — all refs are published documents (FIPS 203/204/205, RFC 9580, etc.).
...
|--------|----------|--------|-------------|
| **BSI** (Germany) | TR-02102-1 | Updated 2025 | PQC for critical infrastructure by **June 2026** (imminent). |
...
**Current consensus (May 2026):** Hybrid key exchange is settled (X25519+ML-KEM-768 deployed at scale). Hybrid *signatures* via composite are now advancing through the standards process — composite ML-DSA sigs cleared IESG. The debate is shifting from "should we do composite?" to "how fast can we publish?"
...
- Merkle Tree Certificates (most promising -- <800 byte proofs; v-04 May 2026)
...
| Vendor | Status |
|--------|--------|
| **Cloudflare** | 60% of human TLS traffic uses hybrid ML-KEM. **Full PQ SASE** (TLS + MASQUE + IPsec) as of Feb 2026. PQ automatic for all origins by Jun 2026. 6M+ domains. Zero Trust PQC features expected Jun 2026. |
| **Chrome** | X25519+ML-KEM default. ML-KEM disable option removed in Chrome 138. |
| **Firefox** | X25519MLKEM768 hybrid since Firefox 135. |
| **Edge** | ML-KEM hybrid PQ TLS since Edge 131. |
| **Safari** | PQ TLS 1.3 key exchange in macOS Tahoe 26 / iOS 26 / visionOS 26. |
| **Akamai** | PQ mid-tier connections completed across all networks Q1 2026. |
| **AWS** | ML-KEM PQ TLS in KMS, ACM, Secrets Manager. ML-DSA keys via KMS APIs (GA). |
| **Microsoft** | PQC APIs GA in Windows Server 2025, Windows 11. PQC in ADCS targeted early 2026. |
| **OpenSSH** | sntrup761x25519 default ~5 years. ML-KEM hybrid in RFC Ed Queue. |

### Signatures / Certificates (NOT Yet Deployed)

- No public PQ certificates in production as of May 2026.
- First PQ certificates expected 2026; broad browser trust unlikely before 2027.
- **Merkle Tree Certificates (MTC):** Chrome will NOT use ML-DSA in traditional X.509. MTCs are Chrome's "preferred (or only) option." Phase 1 experiment with Cloudflare underway. Phase 2 (CT Log operators) Q1 2027. Phase 3 (Chrome CQRS) Q3 2027.
- Signature migration is less urgent: active quantum adversary needed (vs. passive harvest-now for KEMs).
- **CA/Browser Forum SC-081v3:** Certificate validity now at 200 days (Mar 2026), shrinking to 100 days (Mar 2027), 47 days (Mar 2029).
...
| **Germany BSI** | PQC for critical infrastructure by **June 2026** (imminent) |
...
| **CA/B Forum SC-081v3** | Cert validity: 200 days (Mar 2026) → 100 days (Mar 2027) → 47 days (Mar 2029) |
...
|---|-----------|---------|--------|-------|----------|
| 1 | draft-ietf-tls-ecdhe-mlkem | TLS | RFC Ed Queue, v-04 | Standards | **Production** |
| 2 | draft-ietf-tls-mlkem | TLS | Stalled (WGLC issues), v-07 | Informational | Stalled |
| 3 | draft-ietf-tls-mldsa | TLS | **IETF Last Call**, v-03 | Informational | High |
| 4 | draft-ietf-tls-hybrid-design | TLS | RFC Ed Queue, v-16 | Informational | High |
...
| 21 | draft-ietf-lamps-cms-composite-kem | LAMPS | **WG Last Call**, v-01 | Standards | Mid |
...
| 30 | draft-ietf-ipsecme-ikev2-mlkem | IPSECME | **AD Evaluation**, v-05 | Standards | High |
...
*Generated 2026-05-26. Sources: IETF Datatracker, Cloudflare blog, DJB blog series, LWN, PKI Consortium, NIST CSRC, ETSI, ISO, 3GPP, various WG mailing lists.*
